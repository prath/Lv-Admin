  ...
  <div class="column site-content">
    <div class="container-fluid">
      ...
      <PageTitleDefault :actionButton="{ title: 'Add Host', url: '/add-user' }">
        <template><h3>Active Guests</h3></template>
        <template #subtitle><p>List of Lokaven Guests</p></template>
      </PageTitleDefault>
      <!-- /end page title -->
...
export default {
  ...
  computed: {
     ...
     */
    ...mapState({
      guests: state => state.users.users.guests,
      pagination: state => state.users.pagination.guests,
    ...
    setupTableData () {
      ...
      const sorted = _.map(this.guests, val => {
